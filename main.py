...
ANSWERS = ['Новичок', 'От полугода до 1 года', 'От 1 года до 3х лет', 'От 3х до 5 лет', 'От 5 лет и более', 'Консультацию',
       'Пакет сессий',
       'Коуч.Программа', 'Наставничество', 'Премиальная программа', '💰До 5 000р.', '🔺5 000-10 000р.', '🔹10 000-20 000р.',
       '🔸20 000-50 000р.',
       ...
       '✊500-2 000 руб.', '👍2 000-5 000 руб.', '👌5 000 - 15 000 руб.', '🥺Не вкладываю', 'Вкладываю, но результат 0']
...
ты ему предлагаешь?""", """3️⃣ Какую стоимость своего продукта ты озвучиваешь клиенту?""", """4️⃣ Откуда чаще всего к тебе приходит основная часть клиентов?""",
...
Берем за норму 5 клиентов из 5 после консультации заходят в платную работу""", """8️⃣ Какой бюджет потратил на свое продвижение или поиск клиентов в прошлом месяце?"""]
...
👇           выбери ниже свой ответ"""
            ...
            bot.send_photo(call.message.chat.id, ph)
            markup = types.InlineKeyboardMarkup(row_width=2)
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f"{QUESTIONS[0]}\n\n"
            ...
            bot.send_photo(call.message.chat.id, ph)
            markup = types.InlineKeyboardMarkup(row_width=2)
            switch_button = types.InlineKeyboardButton(text=ANSWERS[23], callback_data='test112')
            switch_button1 = types.InlineKeyboardButton(text=ANSWERS[24], callback_data='test114')
            switch_button2 = types.InlineKeyboardButton(text=ANSWERS[25], callback_data='test116')
            switch_button3 = types.InlineKeyboardButton(text=ANSWERS[26], callback_data='test118')
            switch_button4 = types.InlineKeyboardButton(text=ANSWERS[27], callback_data='test110')
            markup.add(switch_button, switch_button1, switch_button2, switch_button3, switch_button4)
...
Твое количество баллов: {SCORE} из 64!
